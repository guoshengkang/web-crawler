...
def find_table(html):
  ...
  table_reg = re.compile(unicode(table_reg,'utf-8'),re.S)
  table = re.search(table_reg,html).group(1)
  return table

def find_rows(table):
  row_reg = r'(<tr.*?/tr>)' #找到表格的行
  row_reg = re.compile(unicode(row_reg,'utf8'),re.S)
  rows = re.findall(row_reg,table)
  return rows

def find_cells(row):
  cell_reg = r'(<td.*?/td>)' #找到单元格
  cell_reg = re.compile(unicode(cell_reg,'utf8'),re.S)
  cells = re.findall(cell_reg,row)
  return cells

def find_rank(cell):
  rank_reg=r'\<span class=.*?\>(\d+?)\<\/span\>' #找到排名
  rank_reg = re.compile(unicode(rank_reg,'utf8'),re.S)
  rank=re.search(rank_reg,cell).group(1)
  return rank
def find_keyword(cell):
  keyword_reg=r'<a class="list-title".*?>(.+?)</a>'
  keyword_reg = re.compile(unicode(keyword_reg,'utf8'),re.S)
  keyword=re.search(keyword_reg,cell).group(1)
  return keyword
def find_search(cell):
  search_reg=r'\<span class=.*?\>(\d+?)\<\/span\>'
  search_reg = re.compile(unicode(search_reg,'utf8'),re.S)
...
def process(html,output_filename):
  ...
  for row in rows[1:]:
    ...
    rank=find_rank(cells[0])
    keyword=find_keyword(cells[1])
    search=find_search(cells[3])
